...
function TrackToAdd({
  ...
  return (<div className='d-flex align-items-center mt-2'>
    ...
    <div className='d-flex flex-column w-100 m-3'>
      ...
      {onArtistSearchFocused && (<div className='overlay position-fixed t-0 l-0 w-100 h-100 bg-black bg-opacity-50'
                                      onClick={() => setOnArtistSearchFocused(false)}>
      </div>)}

      <div className='position-relative'>
        {onArtistSearchFocused && (<ul className='artist-search-result position-absolute bg-white m-0 p-0 w-100'>
      ...
      <div className='d-flex flex-wrap'>
        {track.artists.map(artist => (<button className='delete-artist my-1' onClick={e => {
          e.preventDefault()
